...
class TestFilterDefinitions:
    ...
    def test_category_organization(self):
        ...
        expected_categories = ["Gameplay", "Ratings", "Dates", "Content", "My Rating"]
...
class TestSQLGeneration:
    ...
    def test_gameplay_filters_use_tags(self):
        """Test gameplay filters use tag-based subqueries"""
        # Tag-based filters should reference game_labels and labels tables
        for filter_id in ["just-tried", "played", "well-played", "heavily-played"]:
            assert "game_labels" in PREDEFINED_QUERIES[filter_id]
            assert "labels" in PREDEFINED_QUERIES[filter_id]
            assert "system_tag" in PREDEFINED_QUERIES[filter_id]

    def test_unplayed_filter_distinguishes_steam(self):
        """Test unplayed filter has Steam vs non-Steam logic"""
        sql = PREDEFINED_QUERIES["unplayed"]
        assert "games.store = 'steam'" in sql
        assert "games.store != 'steam'" in sql
        assert "Never Launched" in sql
    ...
    def test_my_rating_filters(self):
        """Test My Rating filter SQL conditions"""
        assert "priority" in PREDEFINED_QUERIES["has-priority"]
        assert "priority" in PREDEFINED_QUERIES["no-priority"]
        assert "personal_rating" in PREDEFINED_QUERIES["personally-rated"]
        assert "personal_rating" in PREDEFINED_QUERIES["personally-unrated"]
...
class TestFilterValidation:
    ...
    def test_invalid_filter_ids(self):
        ...
        invalid_ids = ["nonexistent", "fake-filter", "invalid", "", "started"]
...
class TestQueryParameterHandling:
    ...
    def test_single_query_parameter(self, client):
        ...
        response = client.get("/library?queries=played")
        assert response.status_code == 200
        assert "played" in response.text.lower() or "Played" in response.text
    ...
    def test_multiple_query_parameters(self, client):
        ...
        assert "played" in content.lower() or "Played" in content
    ...
    def test_mixed_valid_invalid_filters(self, client):
        ...
        response = client.get("/library?queries=played&queries=invalid&queries=just-tried")
        assert response.status_code == 200
        assert "played" in response.text.lower() or "Played" in response.text
    ...
    def test_queries_with_other_filters(self, client):
        ...
        response = client.get("/library?queries=played&search=test&sort=name")
        assert response.status_code == 200

    def test_my_rating_filter_parameters(self, client):
        """Test My Rating filter parameters are accepted"""
        response = client.get("/library?queries=has-priority")
        assert response.status_code == 200
        response = client.get("/library?queries=personally-rated")
        assert response.status_code == 200
...
class TestResultCounting:
    ...
    def test_count_with_multiple_filters(self, client):
        ...
        response_filtered = client.get("/library?queries=played&queries=highly-rated")
